...
const createSchema = z.object({
  ...
  host: z
    ...
    .min(1)
    .max(253)
    .regex(/^[A-Za-z0-9._:\[\]-]+$/, "hostname has invalid characters"),
  ...
  user: z
    ...
    .min(1)
    .max(64)
    .regex(/^[A-Za-z0-9._-]+$/, "ssh user has invalid characters"),
  color: colorSchema,
});
...
export const POST = withAuth(async (user, request: Request) => {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: parsed.error.errors[0]?.message || "Invalid SSH host payload" },
      { status: 400 }
    );
  }
