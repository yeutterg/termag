...
async function connectedTmuxTargets(userId: string) {
  ...
  return {
    sessions: new Set(projects.flatMap((project) =>
...
function attachableSessions(sessions: TmuxDeviceSession[], connected: Awaited<ReturnType<typeof connectedTmuxTargets>>) {
  return sessions
    ...
    .map((session) => ({
      ...session,
      windows: session.windows.filter((window) => window.target && !connected.windows.has(key(session.rootKey, window.target)))
    }))
