...
export const POST = withAuth(async (user, request: Request) => {
  ...
  if (parsed.data.mode === 'active') {
    ...
    const sessions = await prisma.session.findMany({
      where: {
        ...
        OR: [
          ...
          { tmuxName: { startsWith: `${tmuxSessionName}:` } }
        ]
      },
    ...
    if (projectIds.length > 0) await prisma.project.deleteMany({ where: { userId: user.id, id: { in: projectIds } } });
    await deleteSessionRecords(sessions);

    refreshUserProjects(user.id);
    return NextResponse.json({ ok: true, deletedProjects: projects.length, deletedSessions: sessions.length });
  }
...
async function deleteSessionRecords(sessions: Array<{ id: string; projectId: string | null; tabId: string | null }>) {
  ...
  for (const [projectId, projectSessions] of byProject) {
    const tabIds = [...new Set(projectSessions.map((session) => session.tabId).filter((tabId): tabId is string => Boolean(tabId)))];
    ...
    if (tabIds.length > 0) await prisma.tab.deleteMany({ where: { id: { in: tabIds }, projectId } });
    if (sessionIds.length > 0) await prisma.session.deleteMany({ where: { id: { in: sessionIds }, projectId } });
    await updateProjectStatus(projectId);
  }
}
