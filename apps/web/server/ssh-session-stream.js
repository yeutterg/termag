...
// The stream owns one shared SSH subprocess and lives here in the
...
class SshSessionStream {
  ...
  async ensureDbSession() {
    ...
    if (this.dbSessionPromise) {
      return this.dbSessionPromise;
    }
  ...
  async subscribe(ws, cols, rows, options = {}) {
    ...
    if (dbId) {
      try {
        ...
        tail.reverse();
        const dbSlice =
          recentEntryCount > 0 && tail.length > recentEntryCount
            ? tail.slice(0, tail.length - recentEntryCount)
            : recentEntryCount >= tail.length
              ? []
              : tail;
        for (const chunk of dbSlice) {
          sendBinary(ws, chunk.data);
        }
      } catch (err) {
        if (err?.message === "stream torn down during subscribe") {
          throw err;
        }
        console.error("[ssh-stream] scrollback fetch failed:", err?.message || err);
      }
    }
    for (const data of this.recentBuffer) {
      sendBinary(ws, data);
    }
  ...
  broadcastSubscriberCount() {
    ...
    for (const sub of this.subscribers.values()) {
      try {
        sub.ws.send(JSON.stringify({ type: "subscribers", count }));
      } catch {}
    }
  }
  ...
  recomputePtySize() {
    ...
    if (minCols === this.cols && minRows === this.rows) {
      return;
    }
    this.cols = minCols;
    this.rows = minRows;
