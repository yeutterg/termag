...
export function DirectoryBrowser({
  ...
  const defaultRootKey =
    initialRootKey && roots[initialRootKey] ? initialRootKey : rootKeys[0] || "";
  ...
  const [typedPath, setTypedPath] = useState(() =>
    composePath(roots, defaultRootKey, initialRelativePath ?? "")
  );
  const requestSeq = useRef(0);

  const fetchListing = useCallback(
    async (nextRootKey: string, nextRelative: string) => {
      if (!nextRootKey) {
        setListing(null);
        setError("");
        return;
      ...
      try {
        const url = new URL(
          `/api/devices/${encodeURIComponent(deviceName)}/browse`,
          window.location.origin
        );
        url.searchParams.set("rootKey", nextRootKey);
        if (nextRelative) {
          url.searchParams.set("relativePath", nextRelative);
        }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void fetchListing(rootKey, relativePath), 0);
  ...
  function applyTypedPath() {
    ...
    if (!resolved) {
      setError(
        `Path must be inside one of the device roots (${rootKeys.join(", ") || "none configured"}).`
      );
      return;
    }
    setError("");
        ...
        <div className="space-y-2 px-3 py-3">
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] text-muted">
              ...
              {rootKeys.length === 0
                ? "a configured root"
                : rootKeys.map(key => roots[key]).join(" / ")}
              .
            </span>
            <button
              type="button"
          ...
          <div className="max-h-72 overflow-y-auto">
            ...
            {rootKey &&
              listing?.entries.map(entry => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    navigate(rootKey, [relativePath, entry.name].filter(Boolean).join("/"))
                  }
            ...
            {rootKey && listing?.truncated && (
              <div className="px-3 py-1.5 text-[10px] text-muted">
                Folder has more than 500 entries — only the first 500 are shown.
              </div>
            )}
            {error && <div className="px-3 py-2 text-[11px] text-bad">{error}</div>}
          </div>
        </div>
      )}
...
function resolveTypedPath(
  ...
  for (const candidate of candidates) {
    if (cleaned === candidate.rootPath) {
      return { rootKey: candidate.rootKey, relativePath: "" };
    }
