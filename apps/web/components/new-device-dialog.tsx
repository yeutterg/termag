    ...
    <div className="fixed inset-0 z-50 bg-black/35 p-4" onClick={() => onOpenChange(false)}>
      <section className="mx-auto mt-[10vh] max-w-lg rounded-lg border border-line bg-panel p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-muted">Create one agent token per physical device. Revoke this token if that device is lost or retired.</p>
        </div>
        <form onSubmit={createDevice} className="space-y-3">
          ...
          <button className="h-9 rounded-md bg-text px-3 text-sm font-medium text-bg">Create device token</button>
        </form>
        {createdToken && (
