...
const GROUPS: Group[] = [
  ...
  {
    heading: "Agents",
    items: [{ keys: ["mod", "enter"], label: "New tab in current session" }],
  },
  {
    ...
    items: [
      {
        keys: ["mod", "shift", "P"],
        label: "New session",
        note: "Firefox: collides with print preview",
      },
    ...
    items: [
      {
        ...
        note: "Firefox: collides with bookmarks library",
      },
...
const STATUS_LEGEND: Array<{ status: string; label: string; meaning: string }> = [
  { status: "idle", label: statusLabel("idle"), meaning: "Session attached and waiting" },
  {
    ...
    meaning: "Agent is actively producing output",
  },
  ...
  {
    status: "sleeping",
    label: statusLabel("sleeping"),
    meaning: "Agent offline, or session detached",
  },
];
        ...
        <header className="flex items-center justify-between px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted">
              Press <span className="font-mono">?</span> any time to open this list.
            </p>
          </div>
                  ...
                  {group.items.map(item => (
                    ...
                    >
                      <span className="flex min-w-0 flex-col">
                        ...
                        {item.note && (
                          <span className="truncate text-[11px] text-muted">{item.note}</span>
                        )}
                      </span>
