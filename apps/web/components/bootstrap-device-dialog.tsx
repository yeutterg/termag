...
export function BootstrapDeviceDialog({ open, onOpenChange }: BootstrapDeviceDialogProps) {
  ...
  if (!open) return null;
  const expiresInSec = result ? Math.max(0, Math.floor((new Date(result.expiresAt).getTime() - now) / 1000)) : 0;
    ...
    <div className="fixed inset-0 z-50 bg-black/35 p-4" onClick={() => onOpenChange(false)}>
      <section className="mx-auto mt-[10vh] max-w-lg rounded-lg border border-line bg-panel p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-muted">Generates a one-time, 15-minute code. Run the command on the new device — it writes the URL + token to <code className="font-mono text-xs">~/.termag/config.json</code> for you.</p>
        </div>
        {!result && (
          <form onSubmit={generate} className="space-y-3">
            <label className="block">
              <span className="mb-1 block text-xs font-medium text-muted">Device name (optional)</span>
              <input
                name="deviceName"
                type="text"
                autoFocus
          ...
          <div className="space-y-3">
            <div className="rounded-md border border-line bg-bg p-3">
              ...
              <div className="flex items-center gap-2">
                <code className="min-w-0 flex-1 break-all font-mono text-xs">{result.cliCommand}</code>
                <button
                  type="button"
            ...
            <div className="rounded-md border border-line bg-bg p-3">
              <div className="mb-2 text-xs font-medium text-muted">Or paste this URL into <code className="font-mono">termag bootstrap</code></div>
              <div className="flex items-center gap-2">
                <code className="min-w-0 flex-1 break-all font-mono text-xs">{result.claimUrl}</code>
                <button
                  type="button"
            ...
            <div className={expired ? 'text-xs text-bad' : 'text-xs text-muted'}>
              {expired ? 'Expired — generate a new code.' : `Expires in ${formatSec(expiresInSec)}.`}
            </div>
