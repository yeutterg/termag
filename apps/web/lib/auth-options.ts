...
function allowedEmail(): string | null {
  return process.env.TERMINALZ_ALLOWED_EMAIL?.toLowerCase().trim() || null;
}

function devAuthEnabled() {
  return process.env.NODE_ENV !== "production" && process.env.TERMINALZ_DEV_AUTH === "true";
}
  ...
  if (devAuthEnabled()) {
    result.push(
      CredentialsProvider({
        ...
        async authorize() {
          const email =
            process.env.TERMINALZ_DEV_AUTH_EMAIL?.toLowerCase().trim() || "preview@terminalz.local";
